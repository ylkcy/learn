...

{
    ...
    *Object = NULL;
    ...
    //  将句柄表的值转换为句柄表中的下标, 并获取到句柄表中对应的项, 句柄表的分层结构, 0,1,2
    //
	...
	{
		//从句柄表项中去除描述的属性信息 (OBJ_PROTECT_CLOSE | OBJ_INHERIT | OBJ_AUDIT_OBJECT_CLOSE)
        ...
        if ((ObjectHeader->Type == ObjectType) || (ObjectType == NULL)) 
		{
...
#else
			//解码被赋予的权限 (Access) & ~ObpAccessProtectCloseBit 0x02000000L
            GrantedAccess = ObpDecodeGrantedAccess(ObjectTableEntry->GrantedAccess);
            ...
            if ((SeComputeDeniedAccesses(GrantedAccess, DesiredAccess) == 0) || (AccessMode == KernelMode)) 
			{
                ...
                if ( (ObjectTableEntry->ObAttributes & OBJ_AUDIT_OBJECT_CLOSE) && (ObjectInfo != NULL) && (ObjectInfo->AuditMask != 0) && (DesiredAccess != 0)) 
				{      
