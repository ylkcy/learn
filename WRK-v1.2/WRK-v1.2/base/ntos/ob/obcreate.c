...
ObCreateObject (
    ...
    __in ULONG ObjectBodySize,                        //对象体的大小, sizeof(EPROCESS)
...

{
    ...
    try {
        ...
        if (ARGUMENT_PRESENT(ObjectAttributes)) 
		{
			...
			// 初始化对象属性时，指定长度为对象属性大小
            if (ObjectAttributes->Length != sizeof(OBJECT_ATTRIBUTES) || (ObjectAttributes->Attributes & ~OBJ_ALL_VALID_ATTRIBUTES)) 
			{
                Status = STATUS_INVALID_PARAMETER;
			...
			{
                if ((ObjectCreateInfo->Attributes & OBJ_KERNEL_HANDLE) == 0 && PsGetCurrentProcess() != PsInitialSystemProcess) 
				{
                    DbgBreakPoint ();
                }
            }
...

{
	...
	// 创建对象时, 会计算可选头部分 + 对象头 + 对象体大小申请空间
    ObjectHeader = ExAllocatePoolWithTag( PoolType, HeaderSize + ObjectBodySize, (ObjectType == NULL ? 'TjbO' : ObjectType->Key) | PROTECTED_POOL );
    ...
    if (NameInfoSize != 0) 
	{
        ...
        if ( (OwnershipMode == KernelMode)  && (ObjectCreateInfo != NULL) && (ObjectCreateInfo->Attributes & OBJ_KERNEL_EXCLUSIVE)) 
		{
    ...
    ObjectHeader->PointerCount = 1;             // 通过指针访问的次数
    ObjectHeader->HandleCount = 0;              // 通过句柄访问的次数          
    ObjectHeader->Type = ObjectType;            // 设置对象的类型
