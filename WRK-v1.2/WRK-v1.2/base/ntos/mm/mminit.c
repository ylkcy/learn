...
#define MM_MB_MAPPED_BY_PDE (MM_VA_MAPPED_BY_PDE / (1024*1024))
		...
		{
            KeBugCheckEx (INSTALL_MORE_MEMORY, MmNumberOfPhysicalPages, MmLowestPhysicalPage, MmHighestPhysicalPage, 0x100);
        }
...
--*/
{
    ...
    else if (!ARGUMENT_PRESENT (InputMemory)) 
	{
		...
		{
            Memory2 = ExAllocatePoolWithTag (NonPagedPool, sizeof(PHYSICAL_MEMORY_DESCRIPTOR) + sizeof(PHYSICAL_MEMORY_RUN) * (i - 1), 'lMmM');
            if (Memory2 != NULL) 
			{
