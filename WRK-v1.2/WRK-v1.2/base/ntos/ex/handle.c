...

{
	...
	//判断当前中断级别，非分页内存不可运行在APC级别下，如果发生缺页异常，无法修复
    PAGED_CODE();
	...
	//获取句柄值得高30位
    if (((LocalHandle.Index & (LOWLEVEL_COUNT - 1)) == 0) || ((HandleTableEntry = ExpLookupHandleTableEntry(HandleTable, LocalHandle)) == NULL) || !ExpLockHandleTableEntry( HandleTable, HandleTableEntry)) 
	{
        //
        // If we are debugging handle operations then save away the details
        // 如果处于调试状态
