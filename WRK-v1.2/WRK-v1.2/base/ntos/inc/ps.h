...
{
    LIST_ENTRY WorkingSetExpansionLinks;        //串联所有MMSUPPORT结构。链表头位于全局变量MmWorkingSetExpansionHead
    LARGE_INTEGER LastTrimTime;

    MMSUPPORT_FLAGS Flags;
    ULONG PageFaultCount;                       //页面错误次数
    WSLE_NUMBER PeakWorkingSetSize;             //工作集峰值
...
#else
    ULONG_PTR NumberGenericTableElements: 24;                  //描述节点个数
#endif
    PVOID NodeHint;                                            //NodeHint是命中结点指针                      
    PVOID NodeFreeHint;                                        //NodeFreeHint是释放节点指针
