...
{
    LONG_PTR PointerCount;        //通过指针访问该对象的次数 ex: ObReferenceObjectByHandle
    union 
	{
        LONG_PTR HandleCount;     //通过句柄访问该对想的次数
        PVOID NextToFree;
    };
    POBJECT_TYPE Type;            //对象类型            
    UCHAR NameInfoOffset;         //对象名称信息偏移
    UCHAR HandleInfoOffset;       //对象的句柄信息偏移               
    UCHAR QuotaInfoOffset;        //对象配额信息偏移
    UCHAR Flags;

    union 
	{
        POBJECT_CREATE_INFORMATION ObjectCreateInfo;    //对象的创建信息
        PVOID QuotaBlockCharged;  
    };

    PSECURITY_DESCRIPTOR SecurityDescriptor;            //对象的安全描述符信息
    QUAD Body;                                          //对象体 对象相关部分, 如EPEOCESS ETHREAD
